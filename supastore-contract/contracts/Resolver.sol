...
contract RatingsResolver is SchemaResolver {
    ...
    event RatingSubmitted(address indexed user, uint256 indexed productId, bool valid);
    ...
    function onAttest(Attestation calldata attestation, uint256 /*value*/) internal view override returns (bool) {
        ...
        require(_nftContract.ownerOf(productId) == attestation.recipient, "User does not own the product");
        require(!hasRated[attestation.recipient][productId], "Product already rated by this user");
        return true;
    }

    function onRevoke(Attestation calldata attestation, uint256 /*value*/) internal override returns (bool) {
        uint256 productId = abi.decode(attestation.data, (uint256));
        hasRated[attestation.recipient][productId] = false; // Allow re-rating if revoked
